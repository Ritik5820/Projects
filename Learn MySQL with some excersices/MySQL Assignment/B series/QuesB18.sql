-- B18  What is MF Codd Rule of RDBMS Systems  ? 

-- Rule 1: Information Rule
-- The data stored in a database, may it be user data or metadata, must be a value of some table cell. Everything in a database must be stored in a table format.

-- Rule 2: Guaranteed Access Rule
-- Every single data element (value) is guaranteed to be accessible logically with a combination of table-name, primary-key (row value), and attribute-name 
-- (column value). No other means, such as pointers, can be used to access data.

-- Rule 3: Systematic Treatment of NULL Values
-- The NULL values in a database must be given a systematic and uniform treatment. This is a very important rule because a NULL can be interpreted as one 
...
-- The data stored in a database must be independent of the applications that access the database. Any change in the physical structure of a database must not 
...
-- The end-user must not be able to see that the data is distributed over various locations. Users should always get the impression that the data is located at one 
-- site only. This rule has been regarded as the foundation of distributed database systems.

-- Rule 12: Non-Subversion Rule
-- If a system has an interface that provides access to low-level records, then the interface must not be able to subvert the system and bypass security and 
